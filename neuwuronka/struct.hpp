...
template <size_t S>
auto &dot_vector_vector(const Vector<S> &a, const Vector<S> &b, Vector<S> &out)
{
                                   ...
                                   Vector<W> &out)
{
    ...
    for (size_t h = 0; h < H; ++h)
        for (size_t w = 0; w < W; ++w)
            out.vector[w] += m.at(h, w) * v.vector[h];
    return out;
}
